...
export const aiConfig = {
  ...
  dailyBriefTtlSec: Math.max(
    3600,
    parseInt(process.env.AI_DAILY_BRIEF_TTL_SEC || "86400", 10),
  ),
  explainCacheTtlSec: Math.max(
    300,
    parseInt(process.env.AI_EXPLAIN_CACHE_TTL_SEC || "3600", 10),
  ),
  draftCacheTtlSec: Math.max(
    60,
    parseInt(process.env.AI_DRAFT_CACHE_TTL_SEC || "1800", 10),
  ),
  hourlyMax: Math.max(5, parseInt(process.env.AI_ADMIN_HOURLY_MAX || "30", 10)),
  maxTokens: Math.min(
    2048,
    Math.max(256, parseInt(process.env.AI_MAX_TOKENS || "1024", 10)),
  ),
  temperature: Math.min(
    1,
    Math.max(0, parseFloat(process.env.AI_TEMPERATURE || "0.35")),
  ),
  requestTimeoutMs: Math.max(
    5000,
    parseInt(process.env.AI_REQUEST_TIMEOUT_MS || "28000", 10),
  ),
};
...
export const geminiConfig = {
  ...
  maxTokens: Math.min(
    8192,
    Math.max(512, parseInt(process.env.GEMINI_MAX_TOKENS || "8192", 10)),
  ),
  temperature: Math.min(
    1,
    Math.max(0, parseFloat(process.env.GEMINI_TEMPERATURE || "0.4")),
  ),
  requestTimeoutMs: Math.max(
    10000,
    parseInt(process.env.GEMINI_REQUEST_TIMEOUT_MS || "60000", 10),
  ),
};
