...
import { serializeOrderForClient } from "../types/utils/orderClientSerializer";
...
export const getMyOrders = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    const result = await orderReadService.getMyOrders(
      String(req.user!._id),
      skip,
      limit,
      statusStr,
...
export const cancelOrder = catchAsync(
  async (req: AuthRequest, res: Response) => {
    ...
    const reason =
      String(
        (req.body as { reason?: string })?.reason || "Cancelled by customer",
      )
        .trim()
    ...
    const { order, alreadyCancelled } = await customerCancelOrder(
      orderId,
      userId,
      reason,
    );
    ...
    recordOrderMetric("order.cancel.success", {
      userId,
      orderId,
