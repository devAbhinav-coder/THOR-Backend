...
export const submitPublicTestimonial = catchAsync(
...
export const getAdminTestimonials = catchAsync(
  async (_req: Request, res: Response) => {
...
export const approveTestimonial = catchAsync(
  async (req: Request, res: Response) => {
    ...
    await writeAdminAudit(req as AuthRequest, "testimonial.approve", {
      ...
      linkedReviewId: (testimonial as { linkedReviewId?: string })
        .linkedReviewId,
    });
    const hasProductReview = Boolean(
      (testimonial as { linkedReviewId?: string; product?: unknown })
        .linkedReviewId || (testimonial as { product?: unknown }).product,
    );
    sendSuccess(
      res,
      { testimonial },
...
export const updateTestimonial = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (typeof body.keepImages === "string" && body.keepImages.trim()) {
      try {
        keepImages = JSON.parse(body.keepImages) as {
          url: string;
          publicId: string;
        }[];
      } catch {
    ...
    if (body.showOnHome !== undefined) {
      patch.showOnHome =
        body.showOnHome !== "false" && body.showOnHome !== false;
    }
