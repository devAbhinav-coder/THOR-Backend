...
export const sendCustomMarketingEmail = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { subject, messageHtml, audience, userIds, ctaText, ctaLink } =
      req.body as {
        subject?: string;
        messageHtml?: string;
    ...
    if (audience === "selected") {
      ...
      const selectedRecipients = await User.find({
        _id: { $in: userIds },
        isActive: true,
      }).select("_id email");
      const emails = selectedRecipients.map((r) => r.email);
      const chunks = await enqueueBroadcastChunks(
        emails,
        tpl.subject,
        tpl.html,
      );
    ...
    const filter =
      audience === "admins" ? { role: "admin", isActive: true }
      : audience === "users" ? { role: "user", isActive: true }
      : { isActive: true };
    ...
    const totalRecipients = await enqueueBroadcastByUserFilter(
      ...
      () => ({
        subject: tpl.subject,
        html: tpl.html,
        jobIdPrefix: `marketing:${subject.trim().slice(0, 32)}`,
      }),
      400,
    );
...
export const getUserInsights = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userSegment =
      paidOrderCount >= 5 || totalSpent >= 20000 ? "frequent_buyer"
      : paidOrderCount >= 2 ? "repeat_buyer"
      : paidOrderCount >= 1 ? "new_buyer"
      : "prospect";
...
export const processRefund = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (amt > order.total) {
      return next(
        new AppError("Refund amount cannot exceed order total.", 400),
      );
    }
    ...
    if (order.paymentMethod === "razorpay") {
      ...
      return next(
        new AppError("Refund method is required for COD orders.", 400),
      );
    }
...
export const getReturns = catchAsync(async (req: Request, res: Response) => {
  ...
  const filter: Record<string, unknown> = {
    returnStatus: {
      $in:
        status ? [status] : ["requested", "approved", "rejected", "returned"],
    },
  };
...
export const getReturnsInsights = catchAsync(
  async (_req: Request, res: Response) => {
    ...
    sendSuccess(res, {
      ...
      topCustomers: topCustomersRaw.map(
        (c: {
          userId?: unknown;
          name?: string;
          email?: string;
...
export const resolveReturn = catchAsync(async (req: Request, res: Response) => {
  ...
  await writeAdminAudit(
    req,
    `order.return_${newStatus}` as any,
