...
export const createOrder = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    let checkoutItems, checkoutSubtotal, cartIdToDelete, cartCouponId, cartCouponDiscount, productMap;

    if (buyNowItem) {
      ({ checkoutItems, checkoutSubtotal, productMap, cartIdToDelete, cartCouponId, cartCouponDiscount } = await checkoutService.processBuyNowItem(buyNowItem));
    } else {
      ({ checkoutItems, checkoutSubtotal, productMap, cartIdToDelete, cartCouponId, cartCouponDiscount } = await checkoutService.processCartItems(userId));
    }

    const { discount, couponId } = await checkoutService.evaluateCoupon(userId, checkoutSubtotal, couponCode, cartCouponId, cartCouponDiscount);
    ...
    let orderItems;
    try {
      orderItems = await checkoutService.validateAndBuildItems(checkoutItems, productMap);
    } catch (e) {
      return next(e);
    }
    ...
    if (paymentMethod === 'razorpay') {
      const intentData = {
        total, checkoutItems, orderItems, shippingAddress, checkoutSubtotal, discount, shippingCharge, codFee, tax, couponId, notes, cartIdToDelete
      };
      
      const { intentId, razorpayOrder } = await checkoutService.createRazorpayIntent(userId, intentData);
      ...
      return sendSuccess(res, razorpayBody.data as Record<string, unknown>, 'Payment session started', 201);
    }

    const codOrder = await checkoutService.createCodOrder(orderPayload, checkoutItems, cartIdToDelete, couponId);
