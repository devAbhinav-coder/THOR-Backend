...
import {
  assertSubscribeAllowed,
  assertTestPushAllowed,
} from "../services/notifications/notificationAbuseService";
...
export const unsubscribePush = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      await assertSubscribeAllowed(String(req.user!._id));
...
export const subscribeExpoPush = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      await assertSubscribeAllowed(String(req.user!._id));
      await saveExpoPushToken(
        String(req.user!._id),
        req.body as ParsedExpoTokenBody,
      );
...
export const unsubscribeExpoPush = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
...
export const sendTestPushToSelf = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    if (req.user?.role !== "admin") {
      return next(
        new AppError("Only admins can send test push notifications.", 403),
      );
    }
