...
import { paymentVerificationService, PaymentVerifiedOrderDto } from '../services/paymentVerificationService';
...
export const verifyPayment = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  if (idemKey) {
    const cached = await getIdempotentPaymentVerifyResponse(userId, `${idemKey}:${idemScope}`);
    if (cached) {
  ...
  if (paidDuplicate) {
    ...
    const body = { status: 'success', data: { order: dto }, message: 'Payment already verified' };
    if (idemKey) await setIdempotentPaymentVerifyResponse(userId, `${idemKey}:${idemScope}`, 200, body);
  ...
  if (checkoutIntentId) {
    ...
    if (!gotPayLock) {
      ...
      return next(new AppError('Payment verification in progress. Please retry in a few seconds.', 429));
    }

    try {
      await recordIntentVerifyAttempt(checkoutIntentId);

      const intent = await CheckoutPaymentIntent.findOne({ _id: checkoutIntentId, user: req.user!._id })
        .select(CHECKOUT_INTENT_VERIFY_SELECT)
        .maxTimeMS(PAYMENT_QUERY_MAX_MS);
      if (!intent) return next(new AppError('Checkout session not found.', 404));
      ...
      if (intent.consumedAt && intent.createdOrderId) {
        ...
        if (!replayOrder) {
          return next(
            new AppError('This checkout session is closed. If you were charged, contact support with your payment ID.', 400),
          );
        }
      ...
      if (intent.expiresAt < new Date()) {
        return next(new AppError('Checkout session expired. Please return to your cart and try again.', 400));
      }
      if (intent.razorpayOrderId !== razorpayOrderId) {
        return next(new AppError('Payment session does not match this checkout.', 400));
      }
      ...
      if (!orderDto) {
        return next(new AppError('Order was not recorded after payment. Please contact support with your payment ID.', 500));
      }
      ...
      try {
        ...
        logger.error({
          ...
          checkoutIntentId,
          error: sideErr instanceof Error ? sideErr.message : 'side effects failed',
        });
      }
      securityLog('payment.verify_success', { orderId: String(orderDto._id), checkoutIntentId });
      ...
      if (idemKey) await setIdempotentPaymentVerifyResponse(userId, `${idemKey}:${idemScope}`, 200, successBody);
      return sendSuccess(res, { order: orderDto }, 'Payment verified successfully');
    } catch (err) {
      if (err instanceof AppError && err.statusCode < 500) {
        securityLog('payment.verify_failed', { orderId: payLockKey, statusCode: err.statusCode, message: err.message });
      } else {
        logger.error({
  ...
  if (!orderId) {
    return next(new AppError('Either orderId or checkoutIntentId is required.', 400));
  }
  ...
  if (order.paymentMethod !== 'razorpay') return next(new AppError('This order does not use online payment.', 400));
  ...
  if (order.paymentStatus === 'paid') {
    const paidRow = await Order.findById(orderId)
      ...
      .lean()
      .maxTimeMS(PAYMENT_QUERY_MAX_MS);
    const dto = paidRow ? toOrderPaymentDto(paidRow as Record<string, unknown>) : null;
  ...
  try {
    ...
    if (!orderDto) {
      return next(new AppError('Order was not found after payment. Please contact support with your payment ID.', 500));
    }
...
export const prepareOrderPayment = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  const order = await Order.findOne({ _id: orderId, user: req.user!._id })
    .select('_id orderNumber paymentStatus paymentMethod razorpayOrderId total status statusHistory')
    .maxTimeMS(PAYMENT_QUERY_MAX_MS);
  ...
  if (!gotPrepareLock) {
    ...
    return next(new AppError('Payment preparation in progress. Please retry in a few seconds.', 429));
  }
  ...
  try {
    if (order.paymentMethod !== 'razorpay') {
      logger.warn(
        ...
        { orderId: String(order._id), previousMethod: order.paymentMethod, userId },
      );
    ...
    if (!order.razorpayOrderId) {
      ...
      const linked = await Order.findOneAndUpdate(
        ...
        { $set: { razorpayOrderId: razorpayOrder.id, paymentMethod: 'razorpay' } },
      ...
      if (linked) {
        ...
        const refreshed = await Order.findOne({ _id: order._id, user: req.user!._id })
          .select(ORDER_PAYMENT_RESPONSE_SELECT)
          .lean()
          .maxTimeMS(PAYMENT_QUERY_MAX_MS);
        if (!refreshed?.razorpayOrderId) {
          return next(new AppError('Could not prepare payment session. Please retry.', 409));
        }
    ...
    sendSuccess(res, {
      order: orderDto ? toOrderPaymentDto(orderDto as Record<string, unknown>) : order.toJSON(),
      razorpayOrder: {
