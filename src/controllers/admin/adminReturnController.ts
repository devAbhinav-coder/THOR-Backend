...
export const getReturns = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const page  = Math.max(1, parseInt((req.query.page  as string) || '1',  10));
  const limit = Math.min(50, Math.max(1, parseInt((req.query.limit as string) || '20', 10)));
  const status = req.query.status as string | undefined;

  if (status && !(RETURN_STATUS_FILTER as readonly string[]).includes(status)) {
    return next(new AppError(`Invalid return status. Allowed: ${RETURN_STATUS_FILTER.join(', ')}.`, 400));
  }
...
export const getReturnsInsights = catchAsync(async (_req: Request, res: Response) => {
  ...
  const [statusBreakdown, refundedAgg, reasons, topProducts, topCustomersRaw] = await Promise.all([
    ...
    Order.aggregate([
      ...
      {
        $project: {
          _id: 0, userId: '$_id', returnCount: 1, name: '$userDoc.name', email: '$userDoc.email',
        },
      },
    ]),
  ]);
  ...
  const payload = {
    ...
    topProducts: topProducts.map((p) => ({
      productId: String(p._id), name: p.name || 'Product', sku: p.sku || '', returnCount: p.returnCount,
    })),
    topCustomers: (topCustomersRaw as { userId?: unknown; name?: string; email?: string; returnCount: number }[]).map((c) => ({
      userId: String(c.userId ?? ''), name: c.name || '—', email: c.email || '', returnCount: c.returnCount,
    })),
  };

  setCache(RETURNS_INSIGHTS_CACHE_KEY, payload, RETURNS_INSIGHTS_TTL).catch((e: Error) =>
...
export const resolveReturnController = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.params;
  ...
  const { order, newStatus } = await resolveReturn(req, id, action, adminNote);
