...
export const getRevenuePeriodSummaryHandler = catchAsync(async (req: Request, res: Response) => {
  const rawPeriod = String(req.query.period ?? 'year');
  const period: RevenuePeriod = VALID_PERIODS.includes(rawPeriod as RevenuePeriod)
