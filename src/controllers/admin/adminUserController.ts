...
export const getAllUsers = catchAsync(async (req: Request, res: Response) => {
  ...
  const filter: Record<string, unknown> =
    roleQuery === 'admin' ? { role: 'admin' } : roleQuery === 'all' ? {} : { role: 'user' };
  ...
  const [stats] = await User.aggregate<{
  ...
  }>([
    {
      $group: {
        ...
        activeUsers:   { $sum: { $cond: [{ $and: [{ $eq: ['$role', 'user'] }, '$isActive'] }, 1, 0] } },
        inactiveUsers: { $sum: { $cond: [{ $and: [{ $eq: ['$role', 'user'] }, { $not: '$isActive' }] }, 1, 0] } },
        totalAdmins:   { $sum: { $cond: [{ $eq: ['$role', 'admin'] }, 1, 0] } },
        activeAdmins:  { $sum: { $cond: [{ $and: [{ $eq: ['$role', 'admin'] }, '$isActive'] }, 1, 0] } },
        inactiveAdmins:{ $sum: { $cond: [{ $and: [{ $eq: ['$role', 'admin'] }, { $not: '$isActive' }] }, 1, 0] } },
      },
    },
  ]);

  sendSuccess(res, {
    users:  { total: stats?.totalUsers ?? 0,  active: stats?.activeUsers ?? 0,  inactive: stats?.inactiveUsers ?? 0 },
    admins: { total: stats?.totalAdmins ?? 0, active: stats?.activeAdmins ?? 0, inactive: stats?.inactiveAdmins ?? 0 },
  });
});
...
export const toggleUserStatus = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction) => {
  if (!Types.ObjectId.isValid(req.params.id)) return next(new AppError('Invalid user id.', 400));
...
export const updateUserRole = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!actor || actor.role !== 'admin') return next(new AppError('Only admins can change roles.', 403));
  if (String(actor._id) === req.params.id) return next(new AppError('You cannot change your own role.', 403));

  const user = await User.findById(req.params.id);
  if (!user) return next(new AppError('User not found.', 404));
  if (user.role === role) return next(new AppError(`User is already ${role}.`, 400));

  const previousRole = user.role;
  user.role = role;
  await user.save();
  await writeAdminAudit(req, 'user.role.updated', { previousRole, newRole: role }, String(user._id));

  sendSuccess(res, { user: { _id: String(user._id), role: user.role } }, 'User role updated.');
});
...
export const getUserInsights = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    Order.aggregate<{
      ...
      { $match: { user: user._id } },
      {
        $group: {
          ...
          totalSpent: {
            $sum: { $cond: [{ $eq: ['$paymentStatus', 'paid'] }, { $toDouble: '$total' }, 0] },
          },
  ...
  const m = metricsAgg[0] ?? { orderCount: 0, paidOrderCount: 0, totalSpent: 0, lastOrderAt: null };
  const avgOrderValue = m.paidOrderCount > 0 ? m.totalSpent / m.paidOrderCount : 0;
  const userSegment =
    m.paidOrderCount >= 5 || m.totalSpent >= 20000
      ? 'frequent_buyer'
      : m.paidOrderCount >= 2
      ? 'repeat_buyer'
      : m.paidOrderCount >= 1
