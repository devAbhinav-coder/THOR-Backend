...
export const getAllOrders = catchAsync(async (req: Request, res: Response) => {
  ...
  const ALLOWED_STATUSES = ['pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'return_requested', 'returned'] as const;
  ...
  const searchQ = String(req.query.search || '').trim().slice(0, 80);

  if (statusParam && (ALLOWED_STATUSES as readonly string[]).includes(statusParam)) {
    filter.status = statusParam;
  }
  if (paymentStatusParam && (ALLOWED_PAYMENT_STATUSES as readonly string[]).includes(paymentStatusParam)) {
...
export const updateOrderStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const hasTrackingUpdate =
    carrierTrimmed !== undefined || trackingTrimmed !== undefined || urlTrimmed !== undefined;
  ...
  if (!sameStatus) {
    ...
    if (!allowed || !allowed.includes(requested)) {
      return next(
        new AppError(`Invalid status change from ${previousStatus} to ${status}.`, 400)
      );
    }
  }
  ...
  if (status === 'cancelled' && previousStatus !== 'cancelled') {
    ...
    const populated = await Order.findById(cancelledOrder._id).populate('user', 'name email');
    const user = populated?.user as unknown as { _id?: unknown; name?: string; email?: string } | undefined;

    if (populated && user?.email) {
      const tpl = emailTemplates.orderStatusUpdate(user.name || 'Customer', populated.orderNumber, 'cancelled');
      enqueueEmail({ to: user.email, subject: tpl.subject, html: tpl.html }).catch((e: Error) => logger.warn(`Cancel email enqueue failed: ${e.message}`));
      ...
      if (user._id) {
        ...
        notifyUser(
          ...
          cancelCopy.type
        ).catch((e: Error) => logger.warn(`Cancel notify failed: ${e.message}`));
      }
      ...
      notifyAdminsEmail(adminTpl.subject, adminTpl.html).catch((e: Error) => logger.warn(`Admin cancel email failed: ${e.message}`));
    }
  ...
  if (!sameStatus && populated && user?.email) {
    const trackingOpts =
      status === 'shipped'
        ? { carrier: order.shippingCarrier, awb: order.trackingNumber, trackingUrl: order.trackingUrl }
        : undefined;
    ...
    if (user._id) {
      const statusCopy = getOrderStatusUpdateCopy(populated.orderNumber!, status, trackingOpts);
      notifyUser(
        String(user._id),
        statusCopy.title,
        statusCopy.message,
...
export const processRefundController = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const populated = await Order.findById(order._id).populate('user', 'name email');
    const user = populated?.user as unknown as { _id?: unknown; name?: string; email?: string } | undefined;
    const bankDetails = (order as unknown as { returnRequest?: { userBankDetails?: { upiId?: string; accountName?: string; accountNumber?: string; bankName?: string } } }).returnRequest?.userBankDetails;
