...
export const getGiftCategories = catchAsync(
  async (_req: Request, res: Response) => {
    const cats = await (
      Category as unknown as { find: (q: object) => Promise<ICategory[]> }
    ).find({ isGiftCategory: true, isActive: true });
    sendSuccess(res, { categories: cats });
  },
);

// ─── User: Submit gifting request ─────────────────────────────────────────────
export const submitGiftingRequest = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!name || !email || !occasion || !items) {
      return next(
        new AppError("Name, email, occasion and items are required.", 400),
      );
    }
    ...
    if (!itemsParsed?.length) {
      return next(
        new AppError("Please add at least one item to your request.", 400),
      );
    }
...
export const getGiftingRequests = catchAsync(
  async (req: Request, res: Response) => {
    const {
      status,
      page = 1,
...
export const getGiftingRequestById = catchAsync(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    if (!isAdmin && requestUserId !== req.user?._id.toString()) {
      return next(
        new AppError("You are not authorized to view this request.", 403),
      );
    }
...
export const getMyGiftingRequests = catchAsync(
