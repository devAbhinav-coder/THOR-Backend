...
const optionalBooleanFromString = z.preprocess((val) => {
...
const imagesMetaField = z
  ...
  .optional()
  .describe(
    "JSON array mapping each product image to a color (admin FormData)",
  );
  ...
  body: z
    ...
    .superRefine((data, ctx) => {
      ...
      if (
        ...
        !data.applicableCategoryIds?.length
      ) {
        ctx.addIssue({
          code: "custom",
      ...
      if (
        data.scopeType === "subcategories" &&
        !data.applicableSubcategoryIds?.length
      ) {
        ctx.addIssue({
          code: "custom",
      ...
      if (val.status === "shipped") {
        if (!val.shippingCarrier || val.shippingCarrier.trim().length < 2) {
          ctx.addIssue({
            ...
            message: "Courier is required for shipped orders",
        ...
        if (!val.trackingNumber || val.trackingNumber.trim().length < 3) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Tracking/AWB number is required for shipped orders",
            path: ["trackingNumber"],
          });
        }
      }
