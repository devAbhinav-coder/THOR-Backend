...
export const uploadProductImages = multer({
  ...
  limits: { fileSize: 40 * 1024 * 1024, files: 22 },
}).fields([
...
export const processProductImages = async (
  ...
  try {
    ...
    let motionVideoFile: Express.Multer.File | undefined;
    ...
    if (Array.isArray(rawFiles)) {
      ...
      motionVideoFile = map.motionVideo?.[0];
    }
...
export const processAvatar = async (
  ...
  try {
    ...
    const result = await uploadToCloudinary(
      file.buffer,
      "house-of-rani/avatars",
      [{ width: 200, height: 200, crop: "fill", gravity: "face" }],
    );

    (
      req.file as Express.Multer.File & { path: string; filename: string }
  ...
  try {
    ...
    const uploadPromises = files.map((file) => {
      ...
      return uploadToCloudinary(
        file.buffer,
        "house-of-rani/cart-custom-fields",
  ...
  try {
    ...
    for (const file of files) {
      if (file.fieldname.startsWith("heroImage_")) {
        ...
        const result = await uploadToCloudinary(
          file.buffer,
          "house-of-rani/storefront/promo",
        ...
        const result = await uploadToCloudinary(
          file.buffer,
          "house-of-rani/storefront/shop-banner",
          [{ width: 600, height: 800, crop: "limit" }],
        );
        uploaded.shopBannerLeft = {
          url: result.secure_url,
          publicId: result.public_id,
        };
        ...
        uploaded.giftingSecondary[index] = {
          url: result.secure_url,
          publicId: result.public_id,
        };
        ...
        const result = await uploadToCloudinary(
          file.buffer,
          "house-of-rani/storefront/home-gift",
          [{ width: 900, height: 1200, crop: "limit" }],
        );
        uploaded.homeGiftCard[index] = {
          url: result.secure_url,
          publicId: result.public_id,
        };
      } else if (file.fieldname.startsWith("homeEditorialTileImage_")) {
        const index = file.fieldname.replace("homeEditorialTileImage_", "");
        const result = await uploadToCloudinary(
          file.buffer,
          "house-of-rani/storefront/home-editorial",
          [{ width: 900, height: 1200, crop: "limit" }],
        );
    ...
    (
      req as Request & { uploadedStorefrontImages?: typeof uploaded }
    ).uploadedStorefrontImages = uploaded;
    next();
  } catch (err) {
