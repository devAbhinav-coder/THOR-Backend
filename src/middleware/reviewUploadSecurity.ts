...
export function assertReviewUploadSecurity(
  ...
  if (files.length > REVIEW_MAX_IMAGES) {
    return next(
      new AppError(`You can upload at most ${REVIEW_MAX_IMAGES} images per review.`, 400)
    );
  }

  for (const file of files) {
    if (!ALLOWED_MIME_TYPES.has(file.mimetype)) {
      return next(new AppError('Only JPEG, PNG, WebP, or GIF images are allowed.', 400));
    }
    ...
    if (file.size > 3 * 1024 * 1024) {
      return next(new AppError('Each review image must be 3MB or smaller.', 400));
    }
  }
