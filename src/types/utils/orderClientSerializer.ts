...
function toPlainOrder(order: Record<string, unknown>): Record<string, unknown> {
  if (
    typeof (order as { toJSON?: () => Record<string, unknown> }).toJSON ===
    "function"
  ) {
    return (order as { toJSON: () => Record<string, unknown> }).toJSON();
  }
  if (
    typeof (order as { toObject?: () => Record<string, unknown> }).toObject ===
    "function"
  ) {
...
export function serializeOrderForClient(
  ...
  if (plain.coupon != null)
    dto.coupon = normalizeId(plain.coupon) ?? plain.coupon;
  if (plain.customRequestId != null) {
    dto.customRequestId =
      normalizeId(plain.customRequestId) ?? plain.customRequestId;
  }
  ...
  if (Array.isArray(dto.items)) {
    dto.items = (dto.items as Record<string, unknown>[]).map((item) => ({
      ...
      product:
        item.product != null ?
          (normalizeId(item.product) ?? item.product)
        : item.product,
    }));
  }
  ...
  if (
    mode === "list" &&
    Array.isArray(plain.statusHistory) &&
    plain.statusHistory.length > 0
  ) {
    const last = plain.statusHistory[plain.statusHistory.length - 1] as Record<
      string,
      unknown
    >;
...
export function serializeOrdersForClient(
  ...
  return orders.map((o) =>
    serializeOrderForClient(o, { ...options, mode: options?.mode ?? "list" }),
  );
}
