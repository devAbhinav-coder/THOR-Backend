...
const DEFAULT_INTERVAL_MS = Number(process.env.INVENTORY_OUTBOX_POLL_MS || 15_000);
...
export function startInventoryOutboxPoller(): void {
  ...
  if (process.env.INVENTORY_OUTBOX_POLL_ENABLED === 'false') {
    logger.info('Inventory outbox poller disabled (INVENTORY_OUTBOX_POLL_ENABLED=false)');
    return;
  }
  ...
  const tick = async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'inventory outbox poll failed';
  ...
  logger.info(`Inventory outbox poller started (interval ${DEFAULT_INTERVAL_MS}ms)`);
}
