...
const DEFAULT_INTERVAL_MS = Number(
  process.env.GIFTING_OUTBOX_POLL_MS || 15_000,
);
...
export function startGiftingOutboxPoller(): void {
  ...
  const tick = async () => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "gifting outbox poll failed";
  ...
  logger.info(
    `Gifting outbox poller started (interval ${DEFAULT_INTERVAL_MS}ms)`,
  );
}
