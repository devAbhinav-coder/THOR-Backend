...
export const ALLOWED_STATUS_TRANSITIONS: Record<ManagedOrderStatus, ManagedOrderStatus[]> = {
...
async function withOptionalTransaction<T>(fn: (session: ClientSession | null) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if (
      ...
      logger.warn('[adminOrderService] Mongo transactions unavailable — running without transaction');
      return fn(null);
    }
    throw err;
  } finally {
...
async function restoreOrderStock(
  ...
  await Promise.all(
    order.items.map(async (item) => {
      ...
      await incrementVariantStock(pid, item.variant.sku, item.quantity, session ? { session } : {});
...
export async function cancelOrder(
  ...
  return withOptionalTransaction(async (session) => {
    ...
    if (shouldRestock) {
      await restoreOrderStock(order, actorId, `Order ${order.orderNumber} cancelled`, session);
    }
...
export async function customerCancelOrder(
  ...
  const existing = await Order.findOne({ _id: orderId, user: userId })
    .select('status paymentMethod paymentStatus inventoryReserved orderNumber items total')
    .lean();
  ...
  const claimed = await Order.findOneAndUpdate(
    ...
    { new: true, select: 'status paymentMethod paymentStatus inventoryReserved orderNumber items total' }
  );
  ...
  if (!claimed) {
    ...
    if (concurrent?.status === 'cancelled') {
      return { order: concurrent as InstanceType<typeof Order>, alreadyCancelled: true };
    }
    throw new AppError('Order cannot be cancelled at this stage.', 400);
  }
  ...
  const shouldRestock =
    (claimed as unknown as { inventoryReserved?: boolean }).inventoryReserved === true ||
  ...
  if (!finalOrder) throw new AppError('Order not found after cancellation.', 500);
...
export async function processRefund(
  ...
  if (orderCheck.status === 'refunded') throw new AppError('Order is already refunded.', 400);
  ...
  return withOptionalTransaction(async (session) => {
    ...
    type RefundMethod = 'razorpay_auto' | 'cash' | 'bank_transfer' | 'upi_manual';
    order.refundData = {
      amount: amt,
    ...
    if (previousStatus !== 'cancelled') {
      await restoreOrderStock(order, actorId, `Order ${order.orderNumber} refunded`, session);
    }
