...
export const cartRevalidationService = {
  async assertCartReadyForCheckout(userId: string, cart: CartDto): Promise<void> {
    ...
    for (const item of cart.items) {
      ...
      if (item.quantity < minQty) {
        throw new AppError(`Minimum quantity for "${item.productName}" is ${minQty}.`, 400);
      }

      const variants = (product.variants as { sku: string; stock: number; price?: number }[]) || [];
      const variant = variants.find((v) => v.sku === item.variant.sku);
      if (!variant) {
        throw new AppError(`Variant for "${item.productName}" is no longer available.`, 400);
      }

      if (variant.stock < 1) {
        throw new AppError(`"${item.productName}" is currently out of stock.`, 400);
      }
