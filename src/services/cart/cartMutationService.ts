...
async function bumpCartVersion(
  ...
  for (let attempt = 0; attempt < CART_VERSION_MAX_RETRIES; attempt++) {
    ...
    const result = await Cart.findOneAndUpdate(
      cartVersionFilter(userId, version),
...
export const cartMutationService = {
  async addItem(
    ...
    return withCartMutationLock(userId, async () => {
      ...
      recordCartMetric("cart.item.added", {
        userId,
  ...
  async removeItem(userId: string, cartItemId: string): Promise<CartDto> {
    return withCartMutationLock(userId, async () => {
      ...
      const updated = await bumpCartVersion(userId, {
      ...
      const remaining = (updated.items as unknown[] | undefined) ?? [];
      if (remaining.length === 0) {
        await Cart.findOneAndDelete({ user: userId }).maxTimeMS(
          CART_QUERY_MAX_MS,
        );
      }
