import CartEventOutbox, { CartOutboxEventType } from '../../models/CartEventOutbox';
...
function scheduleDispatchCartOutbox(outboxId: string): void {
  ...
  dispatchCartOutboxById(outboxId).catch((err: Error) => {
    logger.warn({ msg: 'cart_outbox_immediate_dispatch_failed', outboxId, error: err.message });
  });
}
...
export async function dispatchCartOutboxById(outboxId: string): Promise<boolean> {
...
export async function processPendingCartOutboxBatch(limit = 25): Promise<number> {
