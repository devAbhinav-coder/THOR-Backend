...
type PopulatedUser =
  { name?: string; avatar?: string; _id?: Types.ObjectId | string } | undefined;
...
type RawReview = {
  ...
  product?:
    | { name?: string; slug?: string; _id?: Types.ObjectId | string }
    | Types.ObjectId
    | string;
  rating?: number;
  title?: string;
...
function resolveUserFields(review: RawReview): {
  name: string;
  avatar?: string;
  badge: string;
} {
 ...
 */
export function serializeReviewForPublic(
  review: RawReview,
  ...
  const helpfulCount =
    typeof review.helpfulCount === "number" ? review.helpfulCount
    : Array.isArray(review.helpfulVotes) ? review.helpfulVotes.length
    : 0;
...
export function serializeReviewsForPublic(
  reviews: RawReview[],
): Record<string, unknown>[] {
...
export function serializeReviewForOwner(
  review: RawReview,
): Record<string, unknown> {
  const raw =
    typeof review.toObject === "function" ?
