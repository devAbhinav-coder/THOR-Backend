...
const TOXIC_PATTERNS = [/\b(kill\s+yourself|kys)\b/i];
...
export function applyModerationToReview(
  ...
  if (result.flags.length > 0) {
    (review as IReview & { moderationFlags?: string[] }).moderationFlags =
      result.flags;
    (review as IReview & { moderationScore?: number }).moderationScore =
      result.score;
