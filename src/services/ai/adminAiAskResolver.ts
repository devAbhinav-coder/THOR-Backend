import {
  formatDayStatsBullets,
  type IstDayOrderStats,
...
export type AskStoreContext = {
  ...
  operatingExpenses?: {
    ...
    topSpendCategories?: Array<{
      label: string;
      totalYear: number;
      category: string;
    }>;
    packingAdsPrintRelated?: Array<{
      label: string;
      totalYear: number;
      category: string;
    }>;
  ...
  channelMix?: {
    ...
    paymentMethodMixLifetime?: Array<{
      method: string;
      orders: number;
      revenueInr: number;
    }>;
  ...
  revenueTrendLast7Days?: Array<{
    date: string;
    revenue: number;
    orders: number;
  }>;
  revenueByMonthLast6?: Array<{
    year?: number;
    month?: number;
    revenueInr?: number;
  }>;
...
function wantsMonth(q: string): boolean {
  return (
    /\b(month|mahine|mahina|mtd|is mahine|this month)\b/.test(q) &&
    !wantsYesterday(q)
  );
}
...
function wantsViews(q: string): boolean {
  return (
    /\b(view|views|traffic|popular|dekha|dekh|visit)\b/.test(q) &&
    !wantsTopSellers(q)
  );
}
...
function wantsTopSellers(q: string): boolean {
  return (
    /\b(sold|selling|bikne|bika|best seller|top seller|units sold|kitna bika)\b/.test(
      q,
    ) ||
    (/\b(sabse|top|best)\b/.test(q) &&
      /\b(bik|sell|product)\b/.test(q) &&
      !/\bview\b/.test(q))
  );
}

function wantsRestock(q: string): boolean {
  return /\b(restock|stock|out of stock|low stock|inventory|priority)\b/.test(
    q,
  );
}
...
function wantsProjection(q: string): boolean {
  return /\b(future|forecast|projection|estimate|project|aage|end of month|mahine end)\b/.test(
    q,
  );
}
...
function wantsGrowthAdvice(q: string): boolean {
  return (
    /\b(kaise|how|badha|badhau|increase|grow|improve|boost|tips|strategy|kya karu|kya karna)\b/.test(
      q,
    ) &&
    /\b(sales|sale|revenue|bikri|customer|service|conversion|business)\b/.test(
      q,
    )
  );
}
...
function wantsMoneyFlow(q: string): boolean {
  return (
    /\b(actual|paisa|money|flow|kitna aaya|kitna kamaya|total business|pura business)\b/.test(
      q,
    ) && !wantsProfit(q)
  );
}
...
export function tryResolveAdminQuestion(
  ...
  if (wantsProfit(q)) {
    ...
    if (topP?.name) {
      bullets.push(
        `Top profit product: ${topP.name} - ${fmtInr(topP.grossProfitInr ?? 0)} profit`,
      );
    }
    return payload(
      "Profit picture (catalog COGS + operating expenses):",
      bullets.slice(0, 6),
      "Profit",
    );
  }
  ...
  if (wantsOperating(q)) {
    ...
    bullets.push(
      `Year total operating: ${fmtInr(op.yearTotalInr ?? 0)} | MTD: ${fmtInr(op.monthToDateInr ?? 0)}`,
    );
    for (const c of (op.topSpendCategories || op.byCategory || []).slice(
      0,
      6,
    )) {
      bullets.push(
        `${c.label}: ${fmtInr(c.totalYear)} (year) - category: ${c.category}`,
      );
    }
    ...
    bullets.push(
      `Detail entries: Admin → Operating expenses (${op.adminPath || "/admin/operating-expenses"})`,
    );
    if (Number(ps.estimatedNetProfitMtdInr ?? 0) < 0) {
      bullets.push("Net MTD is negative - review the top 2 spend categories");
    } else {
      bullets.push(
        "Cost control: try a 10–15% cap on the largest category before increasing ad spend",
      );
    }
    return payload(
  ...
  if (
    wantsProjection(q) ||
    (wantsMonth(q) && /\b(aage|future|end)\b/.test(q))
  ) {
    const m = tp.thisMonth;
    const bullets = [
    ...
    return payload(
      "Month projection (simple run-rate, not guarantee):",
      bullets,
      "Forecast",
    );
  }
  ...
  if (
    wantsYesterday(q) &&
    (wantsOrders(q) || wantsRevenue(q) || q.includes("kitna"))
  ) {
    const y = tp.yesterday;
    if (!y)
      return payload("Yesterday's data could not be loaded.", [
  ...
  if (wantsToday(q) && (wantsOrders(q) || wantsRevenue(q))) {
    const t = tp.today;
    if (!t)
      return payload("Today's data could not be loaded.", [
        "Retry in a moment",
      ]);
    return payload(
  ...
  if (wantsYear(q) && (wantsRevenue(q) || wantsOrders(q))) {
    ...
    for (const row of (ctx.revenueByMonthLast6 || []).slice(-3)) {
      bullets.push(
        `Month ${row.month}/${row.year}: ${fmtInr(row.revenueInr ?? 0)} revenue`,
      );
    }
  ...
  if (wantsMonth(q) && (wantsOrders(q) || wantsRevenue(q))) {
    ...
    bullets.push(
      `Projected month-end (run-rate): ${fmtInr(proj.projectedMonthRevenueInr ?? 0)}`,
    );
    return payload(
  ...
  if (wantsTopSellers(q)) {
    ...
    if (list.length === 0)
      return payload("Top seller data is empty.", [
        "Check paid orders in the system",
      ]);
    const top = list[0];
    const bullets = list
      .slice(0, 8)
      .map(
        (p, i) =>
          `${i + 1}. ${p.name} - ${p.unitsSold ?? 0} units, ${fmtInr(p.revenueInr ?? 0)}`,
      );
    return payload(
      `Best seller: ${top.name} (${top.unitsSold} units).`,
      bullets,
      "Top sellers",
    );
  }
  ...
  if (
    wantsViews(q) ||
    ((q.includes("sabse") || q.includes("zyada")) && q.includes("view"))
  ) {
    const list = ctx.topViewedProductsDetailed || [];
    if (list.length === 0)
      return payload("View data not found.", ["Refresh analytics"]);
    const top = list[0];
    const bullets = list
      .slice(0, 8)
      .map(
        (p, i) =>
          `${i + 1}. ${p.name} - ${p.views ?? 0} views, ${p.sold ?? 0} sold (${p.conversionPercent ?? 0}% conv)`,
      );
    return payload(
      `Top views: ${top.name} (${top.views} views).`,
      bullets,
      "Top views",
    );
  }
  ...
  if (wantsChannel(q)) {
    ...
    for (const p of ch.paymentMethodMixLifetime || []) {
      bullets.push(
        `Payment ${p.method}: ${p.orders} orders, ${fmtInr(p.revenueInr)}`,
      );
    }
    return payload(
      "Online + offline channels (paid orders):",
      bullets.slice(0, 8),
      "Channel mix",
    );
  }
  ...
  if (wantsGrowthAdvice(q)) {
    ...
    if (net < 0) {
      bullets.push(
        `Net MTD ${fmtInr(net)} - control operating costs (${fmtInr(op.monthToDateInr ?? 0)}) first`,
      );
    }
    if (top.length > 0) {
      const weak = top.filter(
        (p) => (p.views ?? 0) >= 15 && (p.conversionPercent ?? 100) < 2,
      );
      if (weak.length > 0) {
        bullets.push(
          `Fix conversion: ${weak
    ...
    if (best?.name)
      bullets.push(`Best seller ${best.name} - stock + similar listings`);
    for (const p of oosList
      .filter((x) => (x.soldCount ?? 0) >= 2)
      .slice(0, 2)) {
    ...
    if (growth < 0)
      bullets.push(
        `Revenue ${growth}% vs last month - festival bundle on top 3 SKUs`,
      );
