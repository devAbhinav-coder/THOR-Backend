...
import { buildLedgerFromProduct, insertLedgerEntries } from './stockLedgerService';
...
const VALID_REASONS = ['purchase', 'sale_return', 'damage', 'manual_correction', 'opening_stock'] as const;

export async function adjustVariantStock(req: AuthRequest, input: StockAdjustmentInput) {
  ...
  if (!Number.isFinite(stockDelta) && !hasFinancialUpdate) {
    throw new AppError('Must provide either a delta or a financial update (costPrice/price).', 400);
  }
  ...
  const product = await runInTransaction(async (session) => {
    ...
    if (stockDelta !== 0) {
      const ledger = await buildLedgerFromProduct(
        ...
        stockDelta,
        {
          reason: reason as 'purchase' | 'sale_return' | 'damage' | 'manual_correction' | 'opening_stock',
