...
import {
  calcPurchaseLineItem,
  roundMoney,
  sumMoney,
} from "../../types/utils/financialMath";
...
function processLineItems(
  ...
  return lineItems.map((li) => {
    ...
    const calc = calcPurchaseLineItem(
      li.quantity,
      li.unitCost,
      gstRate,
      supplyType,
    ...
    return {
      product:
        li.product && Types.ObjectId.isValid(li.product) ?
          li.product
        : undefined,
...
export async function createPurchaseInvoice(
  ...
  if (idempotencyKey) {
    ...
    if (existing) {
      recordInventoryMetric("inventory.purchase_invoice.duplicate", {
        phase: "idempotency",
      });
      return existing;
    }
  ...
  try {
    const invoice = await runInTransaction(async (session) => {
      ...
      for (const li of processedLines) {
        if (li.product && li.sku && li.quantity > 0) {
          const stockAfter = await readVariantStockAfter(
            li.product,
            li.sku,
            session,
          );
    ...
    recordInventoryMetric("inventory.purchase_invoice.failed", {
      requestId: ctx?.requestId,
    });
...
export async function updatePurchaseInvoice(
  ...
  if (body.supplierGstin !== undefined)
    inv.supplierGstin = body.supplierGstin.trim() || undefined;
  ...
  await inv.save();
  await writeAdminAudit(req, "inventory.purchase_invoice.updated", {
