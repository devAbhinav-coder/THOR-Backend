...
export async function getOfferAttributionSummary(bounds: {
    ...
    OfferInteractionEvent.aggregate([
      {
        $match: {
          ...visitMatch,
          eventType: { $in: ["popup_impression", "popup_cta_click"] },
        },
      },
      {
        $group: {
  ...
  const promoNameMap = new Map(
    promoDocs.map((p) => [
      String(p._id),
      String(
        (p as { displayTitle?: string; name?: string }).displayTitle ||
          p.name ||
          "Auto offer",
      ),
    ]),
  ...
  const couponCodeMap = new Map(
    couponDocs.map((c) => [String(c._id), String(c.code || "")]),
  );
  ...
  if (popupSessionKeys.length > 0) {
    ...
    revenueAfterPopup =
      Math.round((popupOrderAgg[0]?.revenue ?? 0) * 100) / 100;
  }

  const popupCountMap = new Map(
    popupCounts.map((r) => [String(r._id), r.count as number]),
  );
  ...
  for (const row of popupByKind as {
    _id: { kind: string; eventType: string };
    count: number;
  }[]) {
    const kind = row._id.kind;
    const existing = kindMap.get(kind) ?? { impressions: 0, ctaClicks: 0 };
    if (row._id.eventType === "popup_impression")
      existing.impressions += row.count;
    if (row._id.eventType === "popup_cta_click")
      existing.ctaClicks += row.count;
