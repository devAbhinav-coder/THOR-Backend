...
import { createOrderFromGiftingQuote, GiftingShippingAddress } from './giftingOrderService';
import { notifyAdmins, notifyUser } from '../notificationService';
import { enqueueGiftingSideEffect } from './giftingNotificationService';
import { serializeGiftingRequest } from '../../utils/giftingDto';
...
export async function updateGiftingRequestAdmin(
  ...
  if (body.status) {
    ...
    assertQuoteFieldsForStatus(body.status, body.quotedPrice ?? request.quotedPrice);
  }

  if (body.status) request.status = body.status;
  if (body.adminNote !== undefined) request.adminNote = body.adminNote?.trim();
  if (body.quotedPrice !== undefined) request.quotedPrice = Number(body.quotedPrice);
  if (body.deliveryTime !== undefined) request.deliveryTime = body.deliveryTime?.trim();
...
export async function respondToQuote(
  ...
  if (action === 'accept') {
    ...
    if (
      ...
      throw new AppError('A valid shipping address is required to accept the quote.', 400);
    }
    ...
    const { orderId, orderNumber, idempotentReplay } = await createOrderFromGiftingQuote(
      req,
      id,
      shippingAddress,
      idempotencyKey
