...
function buildGiftableFilter(params: {
  ...
  if (params.giftOccasion) filter.giftOccasions = { $in: [params.giftOccasion] };
...
async function refreshRandomPool(filter: Record<string, unknown>): Promise<string[]> {
...
function pickRandomIds(pool: string[], limit: number, excludeIds: string[]): string[] {
...
async function findRandomGiftable(
  ...
  if (pickedIds.length < limit) {
    ...
    const skip = remaining > 0 ? Math.floor(Math.random() * (remaining + 1)) : 0;
    const fallback = await productRepository.findGiftable(randomFilter, skip, limit);
    const fallbackIds = fallback.map((p) => String((p as { _id: unknown })._id));
  ...
  recordGiftingTiming('gifting.products.random_ms', Date.now() - started, { count: products.length });
  return { products: normalizeProducts(products as Record<string, unknown>[]), total };
}
...
export async function getGiftCategories() {
  ...
  return CategoryModel.find({ isGiftCategory: true, isActive: true })
    .select('name slug image description subcategories isActive isGiftCategory giftType minOrderQty')
    .lean()
    .maxTimeMS(GIFTING_QUERY_MAX_MS);
}
