...
import { clearCouponAbuseCounter, isCouponValidationThrottled, recordFailedCouponAttempt } from './couponAbuseService';
...
export const couponValidationService = {
  ...
  async validateForCheckout(
    ...
    if (await isCouponValidationThrottled(userId, ip)) {
      throw new AppError('Too many invalid coupon attempts. Please try again later.', 429);
    }
    ...
    const validity = evaluateCouponValidity(coupon, userId, orderAmount, { completedOrders });
    if (!validity.valid) {
      await recordFailedCouponAttempt(userId, ip, normalized);
  ...
  async getEligibleCoupons(
    ...
    for (const coupon of coupons) {
      const validity = evaluateCouponValidity(coupon, userId, orderAmount, { completedOrders, now });
  ...
  async evaluateCouponForOrder(
    ...
    if (couponCode) {
      ...
      if (coupon) {
        const completedOrders = await getUserDeliveredOrderCount(userId);
        const validity = evaluateCouponValidity(coupon, userId, checkoutSubtotal, { completedOrders });
        if (validity.valid) {
          return {
