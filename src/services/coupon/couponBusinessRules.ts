...
export const COUPON_QUERY_MAX_MS = Number(
  process.env.COUPON_QUERY_MAX_MS || 5000,
);
...
export function linesScopeFingerprint(
...
export function lineMatchesCouponScope(
  ...
  if (scope === "categories") {
    if (
      line.categoryId &&
      idSet(coupon.applicableCategoryIds).has(String(line.categoryId))
    ) {
      return true;
    }
    const names = new Set(
...
export function evaluateCouponValidity(
  ...
  if (
    coupon.minCompletedOrders &&
    completedOrders < coupon.minCompletedOrders
  ) {
    return {
      valid: false,
  ...
  if (
    coupon.maxCompletedOrders !== undefined &&
    completedOrders > coupon.maxCompletedOrders
  ) {
    return {
      valid: false,
...
export function assertCouponBusinessRules(input: {
  ...
  if (
    ...
    throw new Error(
      "maxCompletedOrders cannot be less than minCompletedOrders",
    );
  }
