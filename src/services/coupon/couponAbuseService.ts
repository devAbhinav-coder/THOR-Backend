...
export async function recordFailedCouponAttempt(userId: string, ip: string, code: string): Promise<void> {
...
export async function isCouponValidationThrottled(userId: string, ip: string): Promise<boolean> {
...
export async function clearCouponAbuseCounter(userId: string, ip: string): Promise<void> {
  if (!redisEnabled) return;
