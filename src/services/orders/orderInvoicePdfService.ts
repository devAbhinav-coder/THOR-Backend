...
function labelValue(
  ...
  doc
    .font("Helvetica")
    .text(value, x + labelW, y, { width: PAGE.width / 2 - labelW - 8 });
  return y + 13;
}
...
function standardAddressLines(
  addr: NonNullable<InvoiceOrder["shippingAddress"]>,
): string[] {
  const lines: string[] = [];
  if (addr.house && !isPlaceholderAddressField(addr.house))
    lines.push(addr.house);
  if (addr.street && !isPlaceholderAddressField(addr.street))
...
function drawTotalsBox(
  ...
  const row = (
    ...
    doc
      .fillColor(opts?.green ? "#15803d" : MUTED)
      .text(label, x + 8, cy + 5, { width: 90 });
    doc
      .fillColor(TEXT)
      .text(value, x + 98, cy + 5, { width: boxW - 106, align: "right" });
    if (opts?.borderBottom !== false) {
      doc
        .save()
        .lineWidth(0.5)
        .strokeColor(opts?.bold ? BORDER : "#e5e7eb")
        .moveTo(x, cy + h)
        .lineTo(x + boxW, cy + h)
        .stroke()
        .restore();
  ...
  if ((order.discount || 0) > 0) {
    row("Discount applied", `- ${fmtInr(order.discount || 0)}`, {
      green: true,
    });
  }
  row(
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(TEXT)
    .text("FOR THE HOUSE OF RANI", x, sigY + 8, {
      width: boxW,
      align: "center",
    });
  ...
  if (stampLogo) {
    try {
      doc.image(stampLogo, x + boxW / 2 - 18, sigY + 20, {
        width: 36,
        height: 18,
        fit: [36, 18],
      });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor(MUTED)
    .text("AUTHORIZED SIGNATORY", x, sigY + 48, {
      width: boxW,
      align: "center",
...
export function generateOrderInvoicePdf(order: InvoiceOrder): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const invoiceDate =
      order.invoice?.generatedAt || order.deliveredAt || order.createdAt;
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor(TEXT)
      .text(SELLER.name.toUpperCase(), sellerX, y, {
        width: 231,
        align: "right",
      });
    doc.fontSize(8).font("Helvetica").fillColor(TEXT);
    doc.text(SELLER.address, sellerX, doc.y, {
      width: 231,
      align: "right",
      lineGap: 1,
    });
    doc.text(`Email: ${SELLER.email}`, sellerX, doc.y + 2, {
      width: 231,
      align: "right",
    });
    doc.text(`Ph: ${SELLER.phone}`, sellerX, doc.y + 2, {
      width: 231,
      align: "right",
    });
    doc
      .font("Helvetica-Bold")
      .text(`GSTIN: ${SELLER.gstin}`, sellerX, doc.y + 2, {
        width: 231,
        align: "right",
      });
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor("#374151")
      .text("Tax Invoice", PAGE.left, y);
    doc
      .fontSize(8)
      .font("Helvetica")
      .fillColor(MUTED)
      .text("Original for Recipient", PAGE.left, y + 18);
    ...
    if (inPerson) {
      drawAddressColumn(
        ...
        "Billed To",
        customerName,
        inPersonBillingLines(addr.phone),
      );
    ...
    doc
      .save()
      .rect(PAGE.left, tableTop, PAGE.width, 20)
      .strokeColor(BORDER)
      .lineWidth(0.75)
      .stroke()
      .restore();
    ...
    doc.text("DESCRIPTION", cols.desc + 4, tableTop + 6, {
    ...
    doc.text("NET AMOUNT", cols.net, tableTop + 6, {
      width: colWidths.net,
      align: "right",
    });
    ...
    order.items.forEach((item, i) => {
      ...
      doc.fontSize(9);
      const nameH = doc.heightOfString(item.name, {
        width: colWidths.desc - 8,
      });
      const metaH =
        meta ? doc.heightOfString(meta, { width: colWidths.desc - 8 }) + 2 : 0;
      ...
      doc
        .save()
        .rect(PAGE.left, rowY, PAGE.width, rowH)
        .strokeColor("#d1d5db")
        .lineWidth(0.5)
        .stroke()
        .restore();
      ...
      doc
        ...
        .font("Helvetica")
        .fillColor(MUTED)
        .text(String(i + 1), cols.sn + 4, rowY + 6, { width: colWidths.sn });
      doc
        .fillColor(TEXT)
        .font("Helvetica-Bold")
        .text(item.name, cols.desc + 4, rowY + 5, {
          width: colWidths.desc - 8,
        });
      if (meta) {
        doc
          ...
          .fontSize(8)
          .fillColor(MUTED)
          .text(meta, cols.desc + 4, rowY + 5 + nameH, {
            width: colWidths.desc - 8,
          });
      }
      doc.fontSize(9).font("Helvetica").fillColor(TEXT);
      doc.text(fmtInr(item.price), cols.unit, rowY + 6, {
        width: colWidths.unit,
        align: "right",
      });
      ...
      doc
        .font("Helvetica-Bold")
        .text(fmtInr(item.price * item.quantity), cols.net, rowY + 6, {
          width: colWidths.net,
          align: "right",
        });
    ...
    doc
      .save()
      .rect(PAGE.left, tableTop, PAGE.width, rowY - tableTop)
      .strokeColor(BORDER)
      .lineWidth(0.75)
      .stroke()
      .restore();
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fillColor("#374151")
      .text("Return Policy:", PAGE.left, y, {
        width: PAGE.width,
        align: "center",
      });
    doc
