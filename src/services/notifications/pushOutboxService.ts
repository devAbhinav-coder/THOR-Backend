...
export async function recordPushOutbox(data: PushJobData): Promise<string | null> {
  ...
  try {
    ...
    recordNotificationMetric('push.outbox.dispatch_failure', { phase: 'persist' });
    return null;
  }
}
...
function scheduleDispatchPushOutbox(outboxId: string): void {
  dispatchPushOutboxById(outboxId).catch((err: Error) => {
    logger.warn({ msg: 'push_outbox_immediate_dispatch_failed', outboxId, error: err.message });
  });
}
...
async function executePushDispatch(payload: PushJobData): Promise<void> {
  ...
  await enqueuePush(payload, {
    jobId: payload.notificationId ? `push__${payload.userId}__${payload.notificationId}` : undefined,
  });
...
export async function dispatchPushOutboxById(outboxId: string): Promise<boolean> {
  ...
  try {
    ...
    recordNotificationMetric('push.outbox.dispatch_failure', { terminal, attempts });
    return false;
  }
}

export async function processPendingPushOutboxBatch(limit = 25): Promise<number> {
