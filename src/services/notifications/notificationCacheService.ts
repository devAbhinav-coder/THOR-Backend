...
export async function getNotificationCacheVersion(userId: string): Promise<number> {
...
export async function bumpNotificationCacheVersion(userId: string): Promise<number> {
  if (!redisEnabled) return 0;
...
export async function deleteUserNotificationDetailCache(userId: string): Promise<void> {
