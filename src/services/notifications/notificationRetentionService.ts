...
const DEFAULT_RETENTION_DAYS = Number(process.env.NOTIFICATION_RETENTION_DAYS || 180);
export async function archiveStaleNotifications(): Promise<number> {
  ...
  if (modified > 0) {
    logger.info({ msg: 'notification_retention_archived', count: modified, cutoff });
  }
  return modified;
}

export async function purgeCompletedPushOutbox(olderThanDays = 30): Promise<number> {
...
export async function runNotificationRetentionBatch(): Promise<{ archived: number; outboxPurged: number }> {
