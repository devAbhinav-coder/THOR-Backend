...
const MAX_PER_IP = parseInt(
  process.env.REFRESH_TOKEN_IP_HOURLY_MAX || "120",
  10,
);
 ...
 */
export async function assertRefreshAllowed(
  userId: string,
  ip: string,
): Promise<void> {
  ...
  if (i > MAX_PER_IP) {
    ...
    throw new AppError(
      "Too many session refreshes from this network. Try again later.",
      429,
    );
  }
}
