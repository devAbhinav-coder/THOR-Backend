...
import { roundMoney, sumMoney } from '../utils/financialMath';
...
export async function listOperatingExpenses(params: {
  ...
  if (params.category && OPERATING_EXPENSE_CATEGORIES.includes(params.category as OperatingExpenseCategory)) {
...
export async function getOperatingExpenseSummary(params: { year?: number }) {
  ...
  const match = { status: { $ne: 'voided' }, expenseDate: { $gte: startDate, $lt: endDate } };
  ...
  const [byCategory, monthly, recentTotal] = await Promise.all([
    ...
    OperatingExpense.aggregate([
      ...
      { $group: { _id: null, grandTotal: { $sum: '$amount' }, count: { $sum: 1 } } },
    ]).option({ maxTimeMS: INVENTORY_QUERY_MAX_MS }),
  ]);
  ...
  const allTimeGrand = roundMoney((recentTotal[0] as { grandTotal?: number })?.grandTotal ?? 0);
  ...
  const monthToDateTotal = roundMoney((mtdAgg as { total?: number })?.total ?? 0);

  const heatmapCells = (monthly as { _id: { year: number; month: number; category: string }; total: number }[]).map(
    (r) => ({
      year: r._id.year,
      month: r._id.month,
  ...
  return {
    ...
    byCategory: (byCategory as { _id: string; total: number; count: number }[]).map((r) => ({
      category: r._id,
...
export async function updateOperatingExpense(
  ...
  if (!Types.ObjectId.isValid(id)) throw new AppError('Expense not found.', 404);
  const expense = await OperatingExpense.findOne({ _id: id, status: { $ne: 'voided' } });
...
export async function voidOperatingExpense(req: AuthRequest, id: string): Promise<void> {
  if (!Types.ObjectId.isValid(id)) throw new AppError('Expense not found.', 404);
  const expense = await OperatingExpense.findOne({ _id: id, status: { $ne: 'voided' } });
  if (!expense) throw new AppError('Expense not found.', 404);
