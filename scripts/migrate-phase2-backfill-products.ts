...
async function run() {
  ...
  const subcats = await SubCategory.find({}).lean<
    (ISubCategory & { _id: mongoose.Types.ObjectId })[]
  >();

  // Load all Category documents (old ones)
  const allCategories = await Category.find({}).lean<
    (ICategory & { _id: mongoose.Types.ObjectId })[]
  >();
  ...
  for (const subcat of subcats) {
    ...
    if (subcat._migratedFromCategoryId) {
      ...
      if (oldCat) {
        ...
        oldCatToParentId.set(
          oldCat.name.toLowerCase(),
          subcat.categoryId as mongoose.Types.ObjectId,
        );
  ...
  console.log(
    `[Phase 2] Lookup maps built. ${oldCatToParentId.size} category mappings, ${subcatStringToId.size} subcategory mappings.`,
  );
  ...
  let cursor = Product.find({
    ...
    .select("_id name category subcategory categoryId subcategoryId")
    .cursor();
  ...
  for await (const product of cursor) {
    ...
    const subcatId =
      oldCatToSubcatId.get(catKey) ||
      (subKey ? subcatStringToId.get(subKey) : undefined);
